...
This module was written to support the L<Astro::Coord::ECI::TLE>
module, and others which go into the prediction of satellite
visibility. Its main task is to convert the
...
B<Caveat user:> If a subclass of this class models the position of a
body over time, then setting the time B<by any method whatsoever>
will cause the model to be run and the position to be set. But there
is B<no> mechanism to prevent you from overriding the calculation.
So don't, unless you have a good reason to (e.g.) deliberately place
the Sun overhead at noon December 21 at the North Pole.
